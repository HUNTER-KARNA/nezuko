...
from logging import (INFO, WARNING, FileHandler, StreamHandler, basicConfig,
                     getLogger)
from os import environ, listdir, mkdir, path  # Importing path from os
...
version_path = "./Version"  # Renamed from path to version_path
version = []
...
if path.isdir(version_path):
    ...
    LOGGER.error("Directory './Version' does not exist. Please create it or check the setup.")
    sysexit(1)

# Extract the latest version from the version files
VERSION = sorted(version)[-1][8:-3]  # Assuming version file name format is 'versionX.md'
...
client = MongoClient(
    "mongodb+srv://hunterkaran:hY2eMHfPzQ5thH8x@cluster0.14roz.mongodb.net/?retryWrites=true&w=majority",
    tls=True,
